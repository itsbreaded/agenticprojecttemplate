...
Maintain project skill contents only in `.agents/skills`. Make `.claude/skills` a local filesystem alias to that directory so Claude Code and Codex discover the same skills without copied files.
...
3. Validate the alias and show canonical skill entries visible through it.
4. Tell the user to restart a running Claude Code session if this created its first `.claude/skills` directory. Start a new Codex session if skill discovery already occurred.
...
- Run `git status --short`; canonical skill content belongs under `.agents/skills`, while the local alias must not appear as a file tree to commit.
